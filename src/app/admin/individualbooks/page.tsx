...
export default function AdminIndividualBooksPage() {
  ...
  const [reviewBook, setReviewBook] = useState<IndividualBookItem | null>(null);
  ...
  const handleOpenEditModal = (book: IndividualBookItem) => {
    ...
    setFormData({
      ...
      pendingContent: book.pendingContent || "",
      contentStatus: book.contentStatus || (book.content ? "APPROVED" : "DRAFT"),
  ...
  const handleOpenReviewModal = (b: IndividualBookItem) => {
    setReviewBook(b);
  ...
  const handleApproveDraft = async (bookId: string, customContent?: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/individualbooks/${bookId}`, {
        method: "PATCH",
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ text: "Content approved and published to live book summary!", type: "success" });
      ...
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            Manage individual books, pending AI drafts & review workflow, HTML content, and Cloudflare R2 media
          </p>
        </div>
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {books.map((book) => {
                ...
                const contentStatus = book.contentStatus || (book.content ? "APPROVED" : "DRAFT");
                const isQueued = !!book.needsContentGeneration;
                const hasPending = !!(book.pendingContent && book.pendingContent.trim().length > 0);
                const hasLiveContent = !!(book.content && book.content.trim().length > 50);
                ...
                return (
                  <tr key={book._id} style={styles.tr}>
                        ...
                        <div>
                          ...
                          {hasPending && (
                            <span style={{ fontSize: "0.72rem", color: "#F59E0B", fontWeight: 600 }}>
                              ⚡ AI Draft Available ({book.pendingContent!.length} chars)
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <td style={styles.td}>
                      <select
                        value={bookStatus}
                        onChange={(e) => handleStatusChange(book._id, e.target.value as "ACTIVE" | "INACTIVE" | "PENDING")}
                        style={{
                          padding: "4px 8px",
                          borderRadius: "6px",
                          fontSize: "0.75rem",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", borderBottom: "1px solid #334155", paddingBottom: "12px" }}>
              <div>
                ...
                <p style={{ color: "#94A3B8", fontSize: "0.85rem", margin: "4px 0 0 0" }}>
                  Book: <strong style={{ color: "#F8FAFC" }}>{reviewBook.title}</strong> {reviewBook.author ? `by ${reviewBook.author}` : ""} ({reviewBook.topic})
                </p>
              </div>
              <button
              ...
              <div>
                ...
                {reviewDraftText ? (
                  ...
                  <div style={{ padding: "40px", textAlign: "center", color: "#94A3B8", backgroundColor: "#0F172A", borderRadius: "8px" }}>
                    No pending AI draft found for this book.
                  </div>
                )}
              </div>
            )}
        ...
        <div style={styles.modalBackdrop}>
          <div style={styles.modalContent}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "18px" }}>
              <h2 style={{ ...styles.modalTitle, margin: 0 }}>{editingId ? "Edit Individual Book" : "Add New Individual Book"}</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveBook} style={styles.form}>
              ...
              <div style={{ padding: "10px 14px", backgroundColor: "#0F172A", borderRadius: "8px", border: "1px solid rgba(99,102,241,0.25)", display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <label htmlFor="needsContentGeneration" style={{ color: "#CBD5E1", fontSize: "0.85rem", cursor: "pointer" }}>
                  ⚡ <strong>Queue for Automated AI Content Cron</strong> (Scraper will generate content into pendingContent for review)
                </label>
              </div>
